 ...
 * eliminación de instancias de {@link Coche}. Se apoya en la capa de persistencia
 * a través de la interfaz {@link CocheDao} y su implementación {@link JpaCocheDao}
 * </p>
 * 
 * @author John
 */
...
public class ControllerCoche {
     ...
     * </p>
     * <p>
     * Este método recibe el cuerpo de la solicitud en formato JSON que representa
     * un objeto {@link Coche}. Utiliza la librería Gson para convertir el JSON a un objeto
     * Coche, y luego llama al método <code>addCoche</code> del DAO para persistirlo
     * Si la operación se realiza correctamente, se envía una respuesta HTTP 200 con
     * un mensaje de confirmación; de lo contrario, se captura la excepción y se responde
     * con un error HTTP 500 y el mensaje correspondiente
     * </p>
     *
     * @param body     Cadena JSON que contiene la información del coche a agregar
     * @param response Objeto {@link Response} para construir y enviar la respuesta HTTP
     */
    @POST("/api/coche")
     ...
     * </p>
     * <p>
     * Este método procesa una solicitud GET y utiliza el método <code>getAllCoches</code>
     * del DAO para recuperar una lista de todos los objetos {@link Coche}. La respuesta se
     * devuelve en formato JSON. Si ocurre algún error durante la consulta, se envía un estado
     * HTTP 500 junto con el mensaje de error
     * </p>
     *
     * @param response Objeto {@link Response} para construir y enviar la respuesta HTTP
     */
     ...
     * </p>
     * <p>
     * Este método maneja una solicitud PUT cuyo cuerpo contiene en formato JSON la
     * representación actualizada de un objeto {@link Coche}. Se convierte el JSON a un objeto
     * Coche y se invoca el método <code>updateCoche</code> del DAO para actualizarlo en la
     * base de datos. Una vez actualizado, se envía una respuesta HTTP 200; si ocurre algún error,
     * se envía un estado HTTP 500 con el mensaje de error
     * </p>
     *
     ...
     * </p>
     * <p>
     * Este método procesa una solicitud DELETE cuyo cuerpo contiene un objeto {@link Coche} en
     * formato JSON, identificando el coche a eliminar. Se convierte el JSON a un objeto Coche y
     * se llama al método <code>delete</code> del DAO para realizar la eliminación
     * Si la operación es exitosa, se envía una respuesta HTTP 200; de lo contrario, se envía un
     ...
     * </p>
     * <p>
     * Este método extrae el parámetro de ruta <code>matricula</code> de la URL y utiliza
     * el método <code>findByMatricula</code> del DAO para buscar el coche en la base de datos
     * Si se encuentra el coche, se devuelve en formato JSON; en caso contrario, se responde con
     * un error HTTP 500 junto con el mensaje de error
     * </p>
     *
     * @param response  Objeto {@link Response} para construir y enviar la respuesta HTTP
     * @param matricula Parámetro de la ruta que especifica la matrícula del coche a buscar
     */
